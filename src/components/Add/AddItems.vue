...
const deleteOptions=[
    {name:'Удалить еду',value:'add-food'},
    {name:'Добавить категорию',value:'add-category'}
]
...
const checkFields=()=>{
if(uploadFile?.value && foodName?.value && selectedCategory?.value?.name &&foodPrice?.value &&foodWeight?.value && foodiDCTMax?.value && selectedContainerName?.value?.name && foodContainerCount?.value){
return true
}else{
    return false
}
