...
<Button label="+"  @click="increaseCount(food.value)"/>
        ...
        <div class="flex justify-content-end gap-2">
            ...
            <Button type="button" label="Добавить" @click='createPickupOrder'></Button>
        </div>
    </div>
    <Toast/>
</template>
...
const createPickupOrder =async()=>{
    ...
    if(orderValues.value.comment?.length && orderValues.value.prepareFor?.length  && orderValues.value.userName?.length && orderValues.value.userPhone?.length  && foods?.length>0
 ...
 const response = await http.post('pickup-orders/create-pickup-order',body)
