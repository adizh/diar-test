...
<Button label="+" @click="increaseOrderValues(food)"/>
            ...
            placeholder="Выбрать еду" class="w-full md:w-14rem" id="category"  optionLabel="name"  v-model.trim="newFoods" 
            @change="selectFood"
            />
...
<Button label="+"  @click="increaseCount(food.value)"/>
        ...
        <div class="flex flex-column gap-3 mb-5">
            ...
            <InputText id="comment" class="flex-auto" autocomplete="off" v-model.trim="orderValues.prepareFor" />
        </div>
        <div class="flex flex-column gap-3 mb-5">
    ...
    <div class="flex justify-content-end gap-2">
        ...
        <Button type="button" label="Добавить" @click='updatePickupOrder'></Button>
    </div>
   </section>
    <Toast/>
</template>
...
return food && food !==null
    ...
    if(orderValues.value.prepareFor?.length  && orderValues.value.userName?.length && orderValues.value.userPhone?.length  && totalFoods?.length>0
    &&orderValues?.value?.dishesCount>0 && orderValues?.value?.price>0
    ){
        try{
            const body={
 ...
 if(response.status===200){
    toast.add({severity:'success',summary:"Успешно",detail:"Заказ успешно изменен!"});
