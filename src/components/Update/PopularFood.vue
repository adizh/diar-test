<template>
  <div>
    <Button
      label="Изменить статус еды"
      severity="secondary"
      rounded
      @click="isPopularOpen = true"
    />
    <Dialog
      v-model:visible="isPopularOpen"
      modal
      header="Изменить статус еды"
      :style="{ width: '35rem' }"
    >
      <div class="flex flex-row justify-content-center">
        <div
          class="flex flex-column justify-content-center gap-3 align-items-center w-full"
        >
          <div class="w-full">
            <MultiSelect
              display="chip"
              ...
              :options="store.getters.getAllFoods"
              filter
              optionLabel="name"
