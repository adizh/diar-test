    ...
    <div class="grid mt-4 justify-content-center gap-3 align-items-center">
      ...
      <div class="flex flex-column gap-3 mb-5">
        ...
        <InputNumber
          ...
          class="flex-auto"
          autocomplete="off"
          v-model.trim="orderValues.sdacha"
        />
      </div>
      ...
      <div class="flex flex-column gap-3 mb-5 col-12">
        ...
        <Dropdown
          ...
          placeholder="Добавить еду"
          filter
          class="w-full md:w-14rem"
          id="category"
          optionLabel="name"
          v-model.trim="newSelectedFood"
          @change="selectFood"
        />
      ...
      <div class="flex flex-column gap-3 mb-5">
        <label for="deliveryPrice" class="font-semibold"
          >Цена за доставку</label
        >
        <InputNumber
          id="deliveryPrice"
          class="flex-auto"
          autocomplete="off"
          v-model.trim="orderValues.deliveryPrice"
        />
      </div>
...
const selectFood = (event: any) => {
  const isExist = selectedFoods.value.findIndex(
    (item: Food) => item.id === event.value.id,
  );
  if (isExist === -1) {
...
onMounted(() => {
  ...
  orderValues.value.deliveryPrice = props?.order?.deliveryPrice
