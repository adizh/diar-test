<template>
  <div>
    ...
    <Button
      label="Отменить самовывоз"
      @click="openCancelOrderPickup"
      severity="warning"
    />
    ...
    <div class="card flex justify-content-center">
      <Dialog
        v-model:visible="isModalVisibleCancel"
        modal
        :header="cancelHeader"
        :style="{ width: '25rem' }"
      >
        ...
        <div class="flex flex-column gap-3 mb-5">
          ...
          <InputNumber
            ...
            class="flex-auto"
            autocomplete="off"
            v-model.trim="orderNumberCancel"
          />
        </div>
        <div class="flex justify-content-end gap-2">
          <Button
            ...
            label="Отменить"
            severity="secondary"
            @click="isModalVisibleCancel = false"
          ></Button>
          <Button
            type="button"
            label="Добавить"
            @click="addOrderCancel"
          ></Button>
        </div>
      </Dialog>
    </div>
...
const sendReqFoorOrder = async (type: string) => {
  if (name.value?.length > 0 && orderNumber.value) {
    try {
      ...
      toast.add({
        severity: "error",
        summary: "Ошибка при создании еды",
...
const cancelOrderSubmit = async (type: string) => {
  if (orderNumberCancel.value) {
    try {
      const response = await http.post(`admin/${type}`, {
        orderNumber: orderNumberCancel.value,
      });
      console.log("response", response);
      if (response.status === 200) {
        toast.add({
          severity: "success",
          summary: "Успешно отменен",
      ...
      toast.add({
        severity: "error",
        summary: "Ошибка при добавлении еды",
        detail: err.response.data.message,
      });
    } finally {
