<template>
    <div>
        ...
        <div class="flex flex-column gap-3 mb-5">
            ...
            <InputText id="email" class="flex-auto" autocomplete="off" v-model="forms.email" />
        </div>

        <div class="flex flex-column gap-3 mb-5">
            <label for="phone" class="font-semibold">Номер телефона</label>
            
            <InputMask id="phone" v-model.trim="forms.phone" mask="+996 (999) 99-99-99"
            placeholder="+996 (700) 11-11-11" />
        </div>

        <div class="flex flex-column gap-3 mb-5">
            <label for="password" class="font-semibold">Пароль</label>
            <InputText id="password" class="flex-auto" autocomplete="off" type="password" v-model="forms.password" />
        </div>
...
const editCourier =async()=>{
  const isError = Object.values(forms.value)?.every((value)=>value?.trim()?.length>0);
...
try{
    ...
    if(response.status===200){
        toast.add({severity:"success",summary:"Успешно",detail:'Курьер изменен!'});
